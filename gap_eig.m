function [omega, E, H, err] = gap_eig(cluster, case_name, exc_name, max_iters, err_lim, varargin)
    path(path, genpath('../fds-client/'));
    if ~isempty(varargin)
        [omega, d_prim, d_dual, s_prim, s_dual, mu, epsilon, E, J, sim] = ...
            get_case(cluster, case_name, exc_name, varargin{1});
    else
        [omega, d_prim, d_dual, s_prim, s_dual, mu, epsilon, E, J, sim] = ...
            get_case(cluster, case_name, exc_name);
    end
    dims = size(epsilon{1});
...
function [omega, d_prim, d_dual, s_prim, s_dual, mu, epsilon, E, J, sim2] = ...
            get_case(cluster, name, exc_name, varargin)
    base_omega = 0.11;
    switch(name)
        case {'1lo', '1med', '1hi'}
            xyz = 'xyz';
...
%                 J{k} = double(J{k});
            ...
            sim = @(omega, J, E) my_sim(cluster, omega, d_prim, d_dual, s_prim, s_dual, ...
                                mu, epsilon, E, J, ...
                                1e6, 1e-6, 'plot', ...
...
function [E, H, err] = my_sim(cluster, omega, d_prim, d_dual, s_prim, s_dual, ...
                                mu, epsilon, E0, J, ...
                                fds_max_iters, fds_err_lim, view_option, ...
                                E_index, E_loc, name)
    subplot 322;
    [E, H, err] = solve(cluster, omega, d_prim, d_dual, s_prim, s_dual, mu, epsilon, E0, J, ...
                        fds_max_iters, fds_err_lim, view_option);
